...
func TestResolve(t *testing.T) {
  ...
  `)
		...
		mockCache.EXPECT().Get("wg|" + model.GetId()).Return(nil)
		mockDatastore.EXPECT().FindLatestAuthorizationModel(gomock.Any(), storeID).Return(model, nil)
		mockCache.EXPECT().Set("wg|"+model.GetId(), gomock.Any(), 10*time.Minute)
  ...
  `)
		...
		mockCache.EXPECT().Get("wg|" + modelID).Return(cachedGraph)
  ...
  `)
		...
		mockCache.EXPECT().Get("wg|" + modelID).Return(nil)
		mockDatastore.EXPECT().ReadAuthorizationModel(gomock.Any(), storeID, modelID).Return(model, nil)
		mockCache.EXPECT().Set("wg|"+modelID, gomock.Any(), 10*time.Minute)
