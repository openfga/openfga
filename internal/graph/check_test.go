...
func TestAvoidSingleflightDeadlockWithinRequest(t *testing.T) {
	ds := memory.New()
	t.Cleanup(ds.Close)
	...
	cycleCheckResolver.SetDelegate(localCheckResolver)
	...
	resp, err := cycleCheckResolver.ResolveCheck(ctx, &ResolveCheckRequest{
		...
		TupleKey:             tuple.NewTupleKey("document:1", "can_view", "user:jon"),
		ResolutionMetadata: &ResolutionMetadata{
			Depth: 25,
		},
	})
...
func TestAvoidSingleflightDeadlockAcrossRequests(t *testing.T) {
	ds := memory.New()
	t.Cleanup(ds.Close)
	...
	var resp1 *ResolveCheckResponse
	var err1 error
	go func() {
		defer wg.Done()

		resp1, err1 = cycleCheckResolver.ResolveCheck(ctx, &ResolveCheckRequest{
			StoreID:              storeID,
			AuthorizationModelID: model.GetId(),
			TupleKey:             tuple.NewTupleKey("document:1", "editor", "user:jon"),
			ResolutionMetadata: &ResolutionMetadata{
				Depth: 25,
			},
		})
	...
	go func() {
		...
		resp2, err2 = cycleCheckResolver.ResolveCheck(ctx, &ResolveCheckRequest{
			StoreID:              storeID,
			AuthorizationModelID: model.GetId(),
