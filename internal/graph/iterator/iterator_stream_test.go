...
func TestIteratorStream(t *testing.T) {
	...
	t.Run("fetchSource", func(t *testing.T) {
		...
		t.Run("fetch_from_source", func(t *testing.T) {
			...
			c <- &Msg{Iter: storage.NewStaticTupleKeyIterator([]*openfgav1.TupleKey{
				{
					Object: "obj:1",
				}})}
			err := stream.fetchSource(context.Background())
	...
	t.Run("Head", func(t *testing.T) {
		...
		t.Run("non_empty_buffer", func(t *testing.T) {
			item := &openfgav1.TupleKey{
				Object: "obj:1",
			}
			dut := &Stream{
				buffer: storage.NewStaticTupleKeyIterator([]*openfgav1.TupleKey{item}),
			}
			item, err := dut.Head(context.Background())
			require.NoError(t, err)
			require.NotNil(t, item)
			require.Equal(t, "obj:1", item.GetObject())
	...
	t.Run("Drain", func(t *testing.T) {
		...
		t.Run("nonEmpty", func(t *testing.T) {
			...
			require.Equal(t, "obj:1", items[0].GetObject())
			require.Equal(t, "obj:2", items[1].GetObject())
	...
	t.Run("SkipToTargetObject", func(t *testing.T) {
		...
		t.Run("skipToMatchingItem", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, "obj:2", item1.GetObject())
			item2, err := dut.buffer.Next(ctx)
			require.NoError(t, err)
			require.Equal(t, "obj:3", item2.GetObject())
		...
		t.Run("skipToNextItem", func(t *testing.T) {
			...
			item1, err := dut.buffer.Next(ctx)
			require.NoError(t, err)
			require.Equal(t, "obj:3", item1.GetObject())
			item2, err := dut.buffer.Next(ctx)
			require.NoError(t, err)
			require.Equal(t, "obj:4", item2.GetObject())
			_, err = dut.buffer.Next(ctx)
			require.ErrorIs(t, err, storage.ErrIteratorDone)
		})
	})
	t.Run("Stop", func(t *testing.T) {
		sourceIter := storage.NewStaticTupleKeyIterator([]*openfgav1.TupleKey{
			{
				Object: "obj:10",
			},
			{
		...
		buf := storage.NewStaticTupleKeyIterator([]*openfgav1.TupleKey{
			{
				Object: "obj:0",
			},
			{
...
func TestNextItemInSliceStreams(t *testing.T) {
	...
	t.Run("error_on_empty", func(t *testing.T) {
		stream1 := &Stream{buffer: storage.NewStaticTupleKeyIterator([]*openfgav1.TupleKey{})}
		stream2 := &Stream{buffer: storage.NewStaticTupleKeyIterator([]*openfgav1.TupleKey{
			{
				Object: "obj:0",
			},
		})}
	...
	t.Run("skip_item", func(t *testing.T) {
		...
		stream3 := &Stream{buffer: storage.NewStaticTupleKeyIterator([]*openfgav1.TupleKey{
			{
				Object: "obj:0",
			},
			{
		...
		t.Run("should_filter_out_drained_producers", func(t *testing.T) {
			...
			for i := 0; i < 5; i++ {
				...
				if i%2 == 0 {
					...
					expectedLen++
					c <- &Msg{Iter: storage.NewStaticTupleKeyIterator(nil)}
				}
