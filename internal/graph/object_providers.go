...
func newRecursiveObjectProvider(ts *typesystem.TypeSystem, logger logger.Logger, ds storage.RelationshipTupleReader) *recursiveObjectProvider {
	return &recursiveObjectProvider{ts: ts, ds: ds, logger: logger}
}
...
func newRecursiveTTUObjectProvider(ts *typesystem.TypeSystem, logger logger.Logger, ttu *openfgav1.TupleToUserset, concurrencyLimit int) *recursiveTTUObjectProvider {
	tuplesetRelation := ttu.GetTupleset().GetRelation()
	computedRelation := ttu.GetComputedUserset().GetRelation()
	return &recursiveTTUObjectProvider{ts: ts, logger: logger, tuplesetRelation: tuplesetRelation, computedRelation: computedRelation, concurrencyLimit: concurrencyLimit}
}
...
func (c *recursiveTTUObjectProvider) Begin(ctx context.Context, req *ResolveCheckRequest) (<-chan usersetMessage, error) {
	...
	leftChans := iterator.NewFanIn(ctx, c.logger, c.concurrencyLimit)
...
func newRecursiveUsersetObjectProvider(ts *typesystem.TypeSystem, logger logger.Logger, concurrencyLimit int) *recursiveUsersetObjectProvider {
	return &recursiveUsersetObjectProvider{ts: ts, logger: logger, concurrencyLimit: concurrencyLimit}
}
