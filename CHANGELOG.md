...
## [Unreleased]
### Added
- Added a built-in `openfga healthcheck` command that probes a running server's health without any external binary. It supports both the gRPC Health Checking Protocol (`--target grpc`, the default) and the HTTP `/healthz` endpoint (`--target http`), reading the same address and TLS configuration (flags, config file, and `OPENFGA_*` environment variables) as the `run` command. The released container images now use this command for their `HEALTHCHECK`. See `cmd/healthcheck`. [#3247](https://github.com/openfga/openfga/pull/3247)

### Removed
- Removed the bundled `grpc_health_probe` binary from the released OpenFGA images (`Dockerfile`, `Dockerfile.goreleaser`) and the `docker-compose.yaml` service healthcheck that invoked it, replacing all of them with the new built-in `openfga healthcheck` command. This drops the transitive CVE exposure the probe binary introduced while keeping the container `HEALTHCHECK` and Compose `depends_on: service_healthy` working. Kubernetes users can alternatively use the native [`livenessProbe.grpc`](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/#define-a-grpc-liveness-probe) field (enabled by default since Kubernetes 1.24, GA since 1.27); note that native probe only supports plaintext gRPC and cannot present TLS client credentials, so deployments requiring gRPC mTLS should use `openfga healthcheck` (which verifies against the configured certificate) or probe the HTTP `/healthz` endpoint. [#3247](https://github.com/openfga/openfga/pull/3247)
