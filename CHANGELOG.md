...
- Added opt-in `SortAsc` to `Read` and `ReadUsersetTuples`, and made `ReadStartingWithUser` honour its existing flag (previously it sorted unconditionally). All iterator types now accurately report `IsOrdered()`. The experimental `weighted_graph_check` `weight2` pruning optimization can now fire when results are sorted. [#3173](https://github.com/openfga/openfga/pull/3173)
