...
- Fixed experimental `weighted_graph_check` incorrectly returning `false` for a single relation with two or more independent edges that recurse into itself (e.g. `member: [user] or member from parent_group or member from child_group`). `CanApplyRecursion` previously picked one of the recursive edges and silently discarded the others; it now detects this ambiguity and disables the optimization, falling back to resolving each recursive edge independently. [#3233](https://github.com/openfga/openfga/issues/3233)
