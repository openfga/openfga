...
## [Unreleased]
### Changed
- Refactored `ServerContext.Run` in `cmd/run` to reduce its cyclomatic complexity by extracting the profiler server, metrics server, core server construction, gRPC serving, and HTTP gateway setup into dedicated methods (`startProfilerServer`, `startMetricsServer`, `buildOpenFGAServer`, `newGRPCServer`, `serveGRPC`, `runHTTPGateway`). This also removes a `goto` and unifies the duplicated HTTP server startup logic. No behavior change. [#1575](https://github.com/openfga/openfga/issues/1575)
