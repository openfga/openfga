...
func (t *TypeSystem) ModelContainsIntersectionOrExclusion() bool {
	for _, relation := range t.allRelations() {
		rw := relation.GetRewrite()
		if RewriteContainsIntersection(rw) || RewriteContainsExclusion(rw) {
			return true
		}
	}
	return false
