...
func (q *ReadQuery) Execute(ctx context.Context, req *openfgav1.ReadRequest) (*openfgav1.ReadResponse, error) {
	...
	queryOpts := storage.NewPaginationOptions(req.GetPageSize().GetValue(), string(decodedContToken))

	tuples, contToken, err := q.datastore.ReadPage(ctx, store, tupleUtils.ConvertReadRequestTupleKeyToTupleKey(tk), queryOpts, storage.NewQueryOptions(req.GetConsistency()))
	if err != nil {
