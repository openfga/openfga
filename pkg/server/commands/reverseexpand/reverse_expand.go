...
func (c *ReverseExpandQuery) Execute(
	...
	err := c.execute(ctx, req, resultChan, false, resolutionMetadata, queryOpts)
	if err != nil {
		return err
	}
...
func (c *ReverseExpandQuery) dispatch(
	...
	return c.execute(ctx, req, resultChan, intersectionOrExclusionInPreviousEdges, resolutionMetadata, queryOpts)
}
	...
	for _, edge := range edges {
		...
		case graph.DirectEdge:
			pool.Go(func(ctx context.Context) error {
				return c.reverseExpandDirect(ctx, r, resultChan, intersectionOrExclusionInPreviousEdges, resolutionMetadata, queryOpts)
			})
		case graph.ComputedUsersetEdge:
			...
			err = c.dispatch(ctx, r, resultChan, intersectionOrExclusionInPreviousEdges, resolutionMetadata, queryOpts)
			if err != nil {
		...
		case graph.TupleToUsersetEdge:
			pool.Go(func(ctx context.Context) error {
				return c.reverseExpandTupleToUserset(ctx, r, resultChan, intersectionOrExclusionInPreviousEdges, resolutionMetadata, queryOpts)
			})
		default:
...
func (c *ReverseExpandQuery) reverseExpandTupleToUserset(
	...
	err = c.readTuplesAndExecute(ctx, req, resultChan, intersectionOrExclusionInPreviousEdges, resolutionMetadata, queryOpts)
	return err
}
	...
	for {
		...
		pool.Go(func(ctx context.Context) error {
			return c.dispatch(ctx, &ReverseExpandRequest{
				...
				Context:          req.Context,
				edge:             req.edge,
			}, resultChan, intersectionOrExclusionInPreviousEdges, resolutionMetadata, queryOpts)
		})
	}
