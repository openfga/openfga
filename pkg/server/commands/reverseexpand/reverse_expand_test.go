...
func TestReverseExpandResultChannelClosed(t *testing.T) {
	...
	mockDatastore.EXPECT().ReadStartingWithUser(gomock.Any(), store, gomock.Any(), gomock.Any()).
		Times(1).
		DoAndReturn(func(_ context.Context, _ string, _ storage.ReadStartingWithUserFilter, _ storage.ConsistencyOptions) (storage.TupleIterator, error) {
	...
	go func() {
		...
		}, resultChan, NewResolutionMetadata(), storage.ConsistencyOptions{Consistency: openfgav1.ConsistencyPreference_UNSPECIFIED})
		...
		t.Run(test.name, func(t *testing.T) {
			...
			go func() {
				...
				err = q.Execute(ctx, &ReverseExpandRequest{
					...
					User:       test.user,
				}, resultChan, resolutionMetadata, storage.ConsistencyOptions{Consistency: openfgav1.ConsistencyPreference_UNSPECIFIED})
