...
func (q *shadowedListObjectsQuery) Execute(
	...
	if q.checkShadowModeSampleRate() {
		...
		go func() {
			q.executeShadowModeAndCompareResults(cloneCtx, cloneReq, cloneRes)
			q.wg.Done() // only used for testing signals
		}()
	}
	return res, err
}

// cloneObj is a utility function that clones an object of type T using JSON marshalling and unmarshalling.
func cloneObj[T any](obj T) (T, error) {
	marshalled, err := json.Marshal(obj) // Marshal the object to JSON.
	if err != nil {
		return obj, err
	}
	var cloned T
	err = json.Unmarshal(marshalled, &cloned) // Unmarshal back to the original type.
	if err != nil {
		return obj, err
	}
	return cloned, nil
...
func calculateDelta(inputMain []string, inputShadow []string) []string {
	mainObjects := make(map[string]bool)
	shadowObjects := make(map[string]bool)
