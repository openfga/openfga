...
func NewBatchCheckCommand(datastore storage.RelationshipTupleReader, checkResolver graph.CheckResolver, typesys *typesystem.TypeSystem, opts ...BatchCheckQueryOption) *BatchCheckQuery {
	cmd := &BatchCheckQuery{
		...
		typesys:             typesys,
		maxConcurrentChecks: 50, // TODO put this where it belongs
...
func (c *BatchCheckQuery) Execute(ctx context.Context, req *openfgav1.BatchCheckRequest) (*graph.ResolveCheckResponse, *graph.ResolveCheckRequestMetadata, error) {
	err := req.Validate()
	if err != nil {
		return nil, nil, err
	}
	...
	resp, checkRequestMetadata, err := NewCheckCommand(
		checkDatastore,                   // has to pass through
		checkResolver,                    // has to pass through
		typesys,                          // also
	...
	// This will pass through direct to CheckCommand
