...
func TestListUsersUnionPanicExpandUnion(t *testing.T) {
	...
	tests := ListUsersTests{
		...
		{
			name: "union_panic_close_channels",
			req: &openfgav1.ListUsersRequest{
				Object:   &openfgav1.Object{Type: "document", Id: "1"},
				Relation: "viewer",
				UserFilters: []*openfgav1.UserTypeFilter{
			...
			// expectedUsers:     []string{},
			// expectedErrorMsg:  ErrPanic.Error(),
			newListUsersQuery: NewListUsersQueryPanicExpandUnionCloseChannels,
		},
	}
...
func NewListUsersQueryPanicExpandUnionCloseChannels(ds storage.RelationshipTupleReader, contextualTuples []*openfgav1.TupleKey, opts ...ListUsersQueryOption) *listUsersQuery {
	l := &listUsersQuery{
		logger:                   logger.NewNoopLogger(),
		resolveNodeBreadthLimit:  serverconfig.DefaultResolveNodeBreadthLimit,
		resolveNodeLimit:         serverconfig.DefaultResolveNodeLimit,
		...
		expandUnionCloseChannels: expandUnionCloseChannels,
		// Panic during expandUnionCloseChannels causes the appliction to hang
