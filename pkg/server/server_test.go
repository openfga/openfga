...
func TestSingleflightCheckResolverDefault(t *testing.T) {
	s := MustNewServerWithOpts(
		WithDatastore(memory.New()),
	)

	// the default (outer most layer) of the CheckResolver
	// composition should always be SingleflightCheckResolver.
	_, ok := s.checkResolver.(*graph.SingleflightCheckResolver)
	require.True(t, ok)
}
