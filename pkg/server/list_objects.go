...
func (s *Server) ListObjects(ctx context.Context, req *openfgav1.ListObjectsRequest) (*openfgav1.ListObjectsResponse, error) {
	...
	q, err := commands.NewListObjectsQueryWithShadowConfig(
		...
		commands.WithListObjectsBufferSize(s.listObjectsPipelineConfig.BufferConfig.Capacity),
		commands.WithListObjectsNumProcs(s.listObjectsPipelineConfig.NumProcs),
		commands.WithListObjectsPipeExtension(
			s.listObjectsPipelineConfig.BufferConfig.ExtendAfter,
			s.listObjectsPipelineConfig.BufferConfig.MaxExtensions,
		),
