...
// Datastore provides a MS SQL Server based implementation of [storage.OpenFGADatastore].
...
func NewWithDB(db *sql.DB, cfg *sqlcommon.Config) (*Datastore, error) {
	...
	// Use ? placeholder format for MS SQL Server.
...
// HandleSQLError processes a MS SQL Server error and converts it into a more specific error type based on the nature of the SQL error.
func HandleSQLError(err error, args ...interface{}) error {
	...
	if errors.As(err, &sqlErr) {
		...
		case 2627, 2601: // Unique constraint violation (primary key or other unique index)
			if len(args) > 0 {
...
func (s *Datastore) ReadAuthorizationModels(ctx context.Context, store string, options storage.ReadAuthorizationModelsOptions) ([]*openfgav1.AuthorizationModel, string, error) {
	...
	if len(modelIDs) > options.Pagination.PageSize {
		token = modelIDs[options.Pagination.PageSize]
		modelIDs = modelIDs[:options.Pagination.PageSize]
	}

	models, err := sqlcommon.GetAuthorizationModels(ctx, s.dbInfo, store, modelIDs)
	if err != nil {
		return nil, "", err
	}
...
func (s *Datastore) CreateStore(ctx context.Context, store *openfgav1.Store) (*openfgav1.Store, error) {
	...
	err := s.stbl.
		...
		Suffix("SELECT id, name, created_at, updated_at FROM store WHERE id = ?", store.GetId()).
		QueryRowContext(ctx).
...
func (s *Datastore) ReadChanges(ctx context.Context, store string, filter storage.ReadChangesFilter, options storage.ReadChangesOptions) ([]*openfgav1.TupleChange, string, error) {
	...
	sb := s.stbl.
		...
		From("changelog").
		Where(sq.Eq{"store": store}).
		Where(fmt.Sprintf("inserted_at < DATEADD(ms, -%d, GETUTCDATE())", horizonOffset.Milliseconds())).
		OrderBy(orderBy)
	...
	return sqlcommon.ScanTupleChanges(rows)
}
...
func (s *Datastore) ReadAssertions(ctx context.Context, store, modelID string) ([]*openfgav1.Assertion, error) {
	...
	return sqlcommon.UnmarshalAssertions(marshalledAssertions, &assertions)
}
...
func (s *Datastore) ListStores(ctx context.Context, options storage.ListStoresOptions) ([]*openfgav1.Store, string, error) {
	...
	for rows.Next() {
		...
		stores = append(stores, &openfgav1.Store{Id: id, Name: name, CreatedAt: timestamppb.New(createdAt), UpdatedAt: timestamppb.New(updatedAt)})
	}

	return sqlcommon.HandlePagination(stores, options.Pagination.PageSize)
}
