...
// This file holds SQLite's SQL spellings of the ast constructs. Spellings that are identical
...
// sqliteColumn maps a logical column to SQLite SQL, owning the full physical mapping.
//
// SQLite's `tuple` table stores the subject discretely across user_object_type /
// user_object_id / user_relation columns (see assets/migrations/sqlite), so each subject field
// is a plain column reference rather than the packed-`_user` decode MySQL and PostgreSQL need.
// (The physical user_type column is not part of the logical view and is never referenced here.)
