...
func TestReadEnsureNoOrder(t *testing.T) {
	...
	iter, err := ds.Read(ctx,
		store, tuple.
			NewTupleKey("doc:", "relation", ""), storage.ConsistencyOptions{Consistency: openfgav1.ConsistencyPreference_UNSPECIFIED})
	defer iter.Stop()
	require.NoError(t, err)
...
`
	...
	changes, _, err := ds.ReadChanges(ctx, "store", "folder", 0, storage.PaginationOptions{PageSize: storage.DefaultPageSize, From: ""})
	require.NoError(t, err)
