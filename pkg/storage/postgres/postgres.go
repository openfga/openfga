...
func initDB(uri string, username string, password string, cfg *sqlcommon.Config) (*pgxpool.Pool, error) {
	...
	if cfg.MaxIdleConns != 0 {
		c.MinIdleConns = int32(cfg.MaxIdleConns) // TODO: should we have a separate config for this?
	}
	...
	// TODO: Set c.MinIdleConns
	if cfg.ConnMaxLifetime != 0 {
		c.MaxConnLifetime = cfg.ConnMaxLifetime
		c.MaxConnLifetimeJitter = cfg.ConnMaxLifetime / 10
	}
...
func (s *Datastore) read(ctx context.Context, store string, tupleKey *openfgav1.TupleKey, options *storage.ReadPageOptions, db *pgxpool.Pool) (*sqlcommon.SQLTupleIterator, error) {
	...
	return sqlcommon.NewSQLTupleIterator(poolGetRows, HandleSQLError)
}
