...
func TestReadEnsureNoOrder(t *testing.T) {
	...
	iter, err := ds.Read(ctx,
		store,
		tuple.NewTupleKey("doc:", "relation", ""), storage.ReadOptions{})
	defer iter.Stop()
...
func TestReadPageEnsureOrder(t *testing.T) {
	...
	tuples, _, err := ds.ReadPage(ctx,
		...
		storage.ReadPageOptions{Pagination: storage.NewPaginationOptions(0, "")})
	require.NoError(t, err)
...
func TestAllowNullCondition(t *testing.T) {
	...
	tuples, _, err := ds.ReadPage(ctx, "store", &openfgav1.TupleKey{}, storage.ReadPageOptions{Pagination: storage.PaginationOptions{
		PageSize: 2,
	}})
	require.NoError(t, err)
...
`
	...
	changes, _, err := ds.ReadChanges(ctx, "store", "folder", storage.ReadChangesOptions{Pagination: storage.PaginationOptions{}}, 0)
	require.NoError(t, err)
