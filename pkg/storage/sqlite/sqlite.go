...
func New(uri string, cfg *sqlcommon.Config) (*SQLite, error) {
	...
	dbInfo := sqlcommon.NewDBInfo(db, stbl, sq.Expr("CURRENT_TIMESTAMP"))
...
func (m *SQLite) CreateStore(ctx context.Context, store *openfgav1.Store) (*openfgav1.Store, error) {
	...
	_, err = m.stbl.
		...
		Values(store.Id, store.Name, sq.Expr("CURRENT_TIMESTAMP"), sq.Expr("CURRENT_TIMESTAMP")).
		RunWith(txn).
		ExecContext(ctx)
	if err != nil {
		return nil, sqlcommon.HandleSQLError(err)
...
func (m *SQLite) ReadChanges(
	...
	sb := m.stbl.
		...
		From("changelog").
		Where(sq.Eq{"store": store}).
		Where(fmt.Sprintf("inserted_at <= CURRENT_TIMESTAMP - INTERVAL %d MICROSECOND", horizonOffset.Microseconds())).
		OrderBy("ulid asc")
