...
func TestWriteValue(t *testing.T) {
	var cases = map[string]struct {
		...
		"list": {
			...
			output: "A,null,true,1111111111,1key:value,",
		},
...
func TestWriteStruct(t *testing.T) {
	var cases = map[string]struct {
		...
		"general": {
			...
			output: "2keyA:valueA,keyB:valueB,",
		},
		"incorrect_value": {
			writer: &validWriter,
			value: MustNewStruct(map[string]any{
...
func TestWriteTuples(t *testing.T) {
	var cases = map[string]struct {
		...
		"with_condition": {
			...
			output: "/document:A#relationA with A 1key:value,@user:A,document:A#relationA with B 1key:value,@user:A",
		},
...
func TestWriteInvariantCheckCacheKey(t *testing.T) {
	...
	var cases = map[string]struct {
		...
		"writes_cache_key": {
			...
			output: "fake_model_id/document:1#viewer with condition_name 1key1:true,@user:anne1key1:true,",
			error:  false,
		},
		"writer_error": {
...
func TestWriteCheckCacheKey(t *testing.T) {
	...
	var cases = map[string]struct {
		...
		"writes_full_cache_key": {
			...
			output: "document:1#can_view@user:annefake_model_id/document:1#viewer with condition_name 1key1:true,@user:anne1key1:true,",
		},
	}
	for name, test := range cases {
		t.Run(name, func(t *testing.T) {
