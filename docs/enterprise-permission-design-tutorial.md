# 企业权限体系设计：从第一性原理到工程实践

> 基于 OpenFGA 项目深度剖析，面向 TOB 产品经理的权限设计方法论

---

## 目录

- [第一章：为什么需要权限系统](#第一章为什么需要权限系统)
- [第二章：权限模型的演进之路](#第二章权限模型的演进之路)
- [第三章：关系型访问控制的第一性原理](#第三章关系型访问控制的第一性原理)
- [第四章：核心概念——用企业场景说话](#第四章核心概念用企业场景说话)
- [第五章：六大权限设计模式](#第五章六大权限设计模式)
- [第六章：三个核心查询——权限系统的 API 设计](#第六章三个核心查询权限系统的-api-设计)
- [第七章：企业级架构考量](#第七章企业级架构考量)
- [第八章：实战——从零设计一个 SaaS 协作平台的权限体系](#第八章实战从零设计一个-saas-协作平台的权限体系)
- [第九章：常见陷阱与最佳实践](#第九章常见陷阱与最佳实践)
- [附录：术语表](#附录术语表)
...
> **类比**：这就像公司按部门发工牌，工牌能开对应楼层的门。但如果你需要"只有项目组成员才能进项目专用会议室"，工牌系统就力不从心了。

### 2.3 第三代：基于属性的访问控制（ABAC）

**核心思想**：基于主体属性、客体属性、环境属性来动态计算权限。
...
关系数据：
  文档:合同A  的 编辑者 是 张三
  文件夹:法务  的 查看者 是 法务部#成员
  文档:合同A  的 父级   是 文件夹:法务
```
...
| 形态 | 格式 | 含义 | 示例 |
|------|------|------|------|
| 直接用户 | `type:id` | 一个具体的人或服务 | `user:张三` |
| 用户集 | `type:id#relation` | 某个对象的某类关系人的集合 | `team:工程部#member`（工程部的所有成员） |
| 通配符 | `type:*` | 某类型的所有实体 | `user:*`（所有用户，即公开访问） |

**用户集是 ReBAC 的精髓所在**——它让你可以说"工程部的所有成员都可以查看这份文档"，而不需要逐个列举。
...
3. 文档的 `viewer` 包括：直接指定的查看者 + 所有编辑者 + 所有所有者
...
掌握了基础概念后，让我们学习 ReBAC 中最常用的六种设计模式。这些模式就像"设计模式"之于编程，是权限设计的"武器库"。

### 模式一：直接授权

**场景**：将某个具体的人指定为某个资源的某个角色。
...
**数据**：
```
document:年报#viewer@user:张三
```

**判断**：
```
Check: user:张三 是 document:年报 的 viewer? ✅ 是
Check: user:李四 是 document:年报 的 viewer? ❌ 否
```

**适用场景**：个人分享、指定审批人、临时授权。

**类比**：就像点对点给某人发微信分享一个文档链接。

---

### 模式二：角色继承（权限层级）

**场景**：所有者自动拥有编辑权限，编辑者自动拥有查看权限。
...
| 角色继承 | `editor or owner` | 权限层级 | owner > editor > viewer |
...
| 策略 | 说明 | 效果 |
|------|------|------|
| 启用查询缓存 | 缓存 Check 结果，减少图遍历 | P50 延迟降低 80% |
| 启用迭代器缓存 | 缓存数据库查询结果 | 数据库负载降低 60% |
| 批量 Check | 用 BatchCheck 合并多个查询 | 减少网络往返 |
| 控制模型深度 | 继承链不超过 5 层 | 避免深度遍历 |
| 直接授权"短路" | 在关键节点加直接关系 | 跳过多层图搜索 |
